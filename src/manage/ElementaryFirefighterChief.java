 ...
 * 2. If the robot is occupied, it refuses to go to the {@code Incendie}
 * advised, else it looks if it's possible top reach the {@code Incendie} from
 * its place. If not, it also refuses.
 * 3. The chosen robot will go to the case of {@code Incendie} and poor his
 * water.
 * 4. If its reservoir is empty, it remains as occupied to the chief and will
 * refuse all the new propositions.
 * 5. The chief go back to steps 1-2-3 on another {@code Incendie} and so on
 * until all the {@code Incendie}s are extincted.
 * NB : In this strategy the robots won't go fill their reservoirs so if they're
 * all empty, the only ones that could extinct the remaining fires would be the
 * "RobotAPattes" (infinite size reservoir).
 */

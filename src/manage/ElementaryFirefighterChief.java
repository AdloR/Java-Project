...
public class ElementaryFirefighterChief extends FireFighterChief {
     ...
     * <li> If the robot is occupied, it refuses to go to the {@code incendie} advised,
     *            else it looks if it's possible top reach the {@code incendie} from its place.
     *            if not, it also refuses.
     *  <li> The chosen robot will go to the case of {@code incendie} and poor his water.
     *  <li> If its reservoir is empty, it remains as occupied to the chief and will
     *            refuse all the new propositions.
     *  <li> The chief go back to steps 1-2-3 on another {@code incendie} and so on until all the
     *            {@code incendie} are extincted.
     * </ul>
     *  NB : in this strategy the robots won't go fill their reservoirs so if they're all empty,
     *            the only ones that could extinct the remaining fires would be the {@code RobotAPattes}
